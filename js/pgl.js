 ...
 * File: pgl.js
 * ------------
 * This file implements a self-contained version of the Portable
 * Graphics Library, which implements a common graphics model
 * across a variety of languages.
 */

// This implementation is a kludge for the reveal.js framework.
// Lots of work is needed to get this polished.

"use strict";

const PGL_VERSION = 0.95;
const PGL_BUGFIX = 2;
const PGL_DATE = "25-Jun-22";
...
/*
 * Implementation notes: _updateMouseEvent
 * ---------------------------------------
 * This function adds getX and getY methods to the mouse event, which
 * return the coordinates of the mouse event relative to the GWindow.
 * This function calculates both the x and y coordinates on the first
 * call, which are cached in the local variables x and y.  The
 * calculation converts screen coordinates to local coordinates
 * using the traditional approach unless the program is running
 * under reveal.js, in which case, it has to implement a more
 * complex strategy to account for scaling.
 */
    ...
    function _updateMouseEvent(e) {
        ...
        let y = null;
        e.getX = function() {
            if (x === null) updateCoordinates();
            return x;
        }
        e.getY = function() {
            if (y === null) updateCoordinates();
            return y;
        }

        function updateCoordinates() {
            let revealFlag = _insideReveal(canvas);
            if (revealFlag) {
                // This strategy may be good for everything
                let sf = Reveal.getScale();
                let slide = Reveal.getSlidesElement();
                let mx = e.clientX;
                let my = e.clientY;
    ...
    function _insideReveal(element) {
        while (element) {
...
/* Protected methods */
        ...
        _getCanvas: _getCanvas,
        _conditionalRepaint: _conditionalRepaint,
        _insideReveal: _insideReveal,
        _paint: _paint,
 ...
 * --------------------
 * The modules in this directory use an object-oriented style recommended
 * by Douglas Crockford that is unfamiliar to many JavaScript programmers,
 * but which has the following advantages:
 *
 * 1. Object state is maintained in the closure of the constructor and is
 *    therefore inaccessible outside the context of the function that
 *    defines the class.
 *
 * 2. The keywords "new" and "this" are not needed in this model, thereby
 *    avoiding one of the most confusing aspects of JavaScript. 
 */

/*
 * Declarations for requirejs
 * --------------------------
 * @Exports:
 *   GWindow
 *   GObject
